...
def main():
    if len(sys.argv) < 2:
        print("Usage Error: sha256_collision.py [a,b,c] (num_bits)")
        return

    if sys.argv[1] == "a":
        user_input = input("Please provide input: ")
        digest = SHA256_hash(user_input.encode("utf-8"))
        salt = digest[0:29]
        hash = digest[29:]
        print('digest: ' + str(hash))
    ...
    if sys.argv[1] == "c":
        if len(sys.argv) != 3:
            print("Usage error: sha256_collision.py c (num_bits)")
            return
